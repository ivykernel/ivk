...
8. **Discard failed attempts.** `ivk ws rm <name>` is cheaper than carrying broken state forward.
9. **Don't share workspaces between agents.** Each agent should get its own `ivk new <agent-id>-<task>`.
10. **`ivk gc` and bulk `ivk ws rm --all/--exported` require `--force` to delete dirty workspaces.** Without `--force`, dirty workspaces are skipped — `ivk` will never silently destroy uncommitted edits. Use `--dry-run` first when unsure.
11. **`ivk gc` never deletes `.ivk/changesets/*.json`.** Changesets are facts about work that happened. If a removed workspace would orphan a changeset, gc reports it under `orphaned_changeset_refs` so the commit can still be inspected via `git show <result_snapshot>`.
