...
> Both pre-Phase spikes (working-tree + build-artifact) **complete**, with
> measured numbers in [`results/`](./results/). Phases 0–9 implemented
> end-to-end. `v0.0.2` shipped via Homebrew tap.
>
> **Working today:**
> - `brew tap ivykernel/tap && brew install ivk` installs the binary
> - `ivk init` / `new` / `ls` / `show` / `diff` / `rm` / `ch new` / `export` / `patch` / `gc` / `doctor` / `help` all functional with `--json` + `--agent` output
> - 44 tests pass, fmt clean, clippy zero warnings, CI green on macOS + Linux
> - LP live at <https://ivykernel.github.io/ivk/> with two live race videos + scaling chart
> - `examples/{demo-task,todo-100}` runnable end-to-end; `design-race`, `agent-fanout` documented as recording recipes
>
> **Remaining 10 unchecked items are all "intentionally deferred from v0.0.x"** with explicit rationale on each (clap, tracing, SQLite, ws mount, ivk ship, ws rm --failed, ws rm --all-discarded, `--from <rev>`, ivk bench matrix, real-agent orchestration scripts). The plan is now genuinely done modulo those future-version items.
...
## Phase 7: 100-agent demo + demo recipe library
...
- [x] live disk race ([`scripts/bench/live-race.sh`](./scripts/bench/live-race.sh) + [`demos/disk-race.tape`](./demos/disk-race.tape)) — htop-style bar visualization, 50× ratio at N=20 pairs
- [x] live cargo-build race ([`scripts/bench/live-build-race.sh`](./scripts/bench/live-build-race.sh) + [`demos/build-race.tape`](./demos/build-race.tape)) — parallel `cargo build --release`, ivk finishes in 0.3s, git worktree in 37s (144×)
- [x] design-race recording plan ([`examples/design-race/PLAN.md`](./examples/design-race/PLAN.md)) — 4-variant browser-side demo with honest framing on what ivk does vs doesn't add at N=4
- [x] agent-fanout recording plan ([`examples/agent-fanout/PLAN.md`](./examples/agent-fanout/PLAN.md)) — 12–30 ws scenario where git worktree becomes painful; recipe for the canonical "this is what ivk is for" recording
...
- [x] create GitHub release — `v0.0.1` and `v0.0.2` both cut natively under `ivykernel/ivk`. v0.0.2 includes the post-transfer URL fixes + the new live-race demos. (The plan originally said v0.1.0; we shipped earlier under v0.0.x to start the feedback loop, with v0.1.0 reserved for the first release that adds a non-`x.x.x → x.x.(x+1)` feature increment.)
- [x] create Homebrew tap (`ivykernel/homebrew-tap`) — public repo bootstrapped via `gh repo create`; cargo-dist's release pipeline pushes `Formula/ivk.rb` updates on every tag via `HOMEBREW_TAP_TOKEN` secret.
- [x] test brew install — `brew tap ivykernel/tap && brew install ivk` verified on macOS; `ivk --version` returns `ivk 0.0.2`.
...
- [x] a new user can install ivk on macOS using Homebrew and run `ivk init` — verified end-to-end:

```bash
brew tap ivykernel/tap
brew install ivk
ivk --version           # ivk 0.0.2
cd /some/git/repo && ivk init --agent-instructions
ivk new attempt-1
```
